...
def get_2d_projection(activation_batch):
    ...
    activation_batch = activation_batch.numpy()
    activation_batch[np.isnan(activation_batch)] = 0
    projections = []
    activations = activation_batch[0]
    reshaped_activations = (activations).reshape(
        activations.shape[0], -1).transpose()
    reshaped_activations = torch.tensor(reshaped_activations)
    # Centering before the SVD seems to be important here,
    # Otherwise the image returned is negative
    reshaped_activations = reshaped_activations - torch.mean(reshaped_activations, dim=0)
    _, _, VT = torch.linalg.svd(reshaped_activations)
...
def get_2d_projection2(activation_batch):
    # TBD: use pytorch batch svd implementation
    activation_batch = activation_batch.numpy()
    activation_batch[np.isnan(activation_batch)] = 0
    projections = []
...
def estimate_integral(path_gradients):
    ...
    # the average.
    return torch.mean(path_gradients, dim=0)


def grad2heatmaps(model, X, gradients, activations=None, operation='iig', score=None, do_nrm_rsz=True, weight_ratio=[]):
