...
class GradModel(nn.Module):
    def __init__(self, model_name='resnet50', feature_layer=8):
        ...
        if model_name == 'resnet101':
            ...
            self.avgpool = model.avgpool
            self.classifier = model.classifier
        elif model_name == 'vgg16-ray-voc':
            self.model_str = 'torchray'
            model_arch = "vgg16"  # @param ['vgg16', 'resnet50']
            model = torchray.benchmark.models.get_model(arch=model_arch, dataset="voc",
                                                        convert_to_fully_convolutional=False)
            self.features = model.features
            self.avgpool = model.avgpool
            self.classifier = model.classifier
        elif model_name == 'vgg16-ray-coco':
            self.model_str = 'torchray'
            model_arch = "vgg16"  # @param ['vgg16', 'resnet50']
            model = torchray.benchmark.models.get_model(arch=model_arch, dataset="coco",
                                                        convert_to_fully_convolutional=False)
            self.features = model.features
            self.avgpool = model.avgpool
            self.classifier = model.classifier
