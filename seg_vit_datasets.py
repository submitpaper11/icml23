...
from datasets.coco import Coco_Segmentation
...
if __name__ == '__main__':
    ...
    if chosen_dataset == 'imagenet':
        ...
        for i, (img, tgt) in enumerate(tqdm(ds)):
            ...
            if i % 300 == 0:
                save_results_to_csv(results, f'iig-imagenet-{i}')
        save_results_to_csv(results, 'iig-imagenet')
        ...
        for i, (img_norm, target, img_resize) in enumerate(tqdm(ds)):
            ...
            results.append(segmentation_results)
            if i % 300 == 0:
                save_results_to_csv(results, f'iig-coco-{i}')
        save_results_to_csv(results, 'iig-coco')
    else:
        print('voc')
        test_img_trans, test_img_trans_only_resize, test_lbl_trans = init_get_normalize_and_trns()
        CWD = os.getcwd()
        ...
        for i, (img, target) in enumerate(tqdm(ds)):
            ...
            results.append(segmentation_results)
            if i % 300 == 0:
                save_results_to_csv(results, f'iig-voc-{i}')
        save_results_to_csv(results, 'iig-voc')
